<template>
  <CollapsibleRegion>
    <template #header>Account Affiliates</template>
...
import {
   getConfigStore, setConfig, type AcbConfig, type AccountBindings, type ConfigStore,
...
function ensureConfig(store: ConfigStore): AcbConfig {
   ...
   return {
      version: 1,
      account_bindings: { questrade: {}, rbc_di: {}, etrade: {} },
   };
}
   ...
   setup() {
      ...
      watch(
         () => {
            ...
            // Depend on file count, kinds, and detecting status so we
            // re-run when files are added/removed or PDF detection completes.
      ...
      // Called when we want to bind, from a 'sibling' element (the button on the row)
