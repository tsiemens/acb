...
impl AcbWriter for CsvWriter {
    fn print_render_table(
        &mut self, out_type: OutputType, name: &str, table_model: &crate::portfolio::render::RenderTable)
        ...
        let file_name = match out_type {
            ...
            OutputType::Costs => format!("{}-costs.csv", name.to_lowercase().replace(" ", "-")),
        };

        let file_path = self.out_dir.join(PathBuf::from(file_name));
        let fp = File::create(file_path.clone())
            .map_err(|e| format!("Failed to create {:?}: {}", file_path.to_str(), e))?;
