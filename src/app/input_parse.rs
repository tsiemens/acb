...
use crate::{
    portfolio::{PortfolioSecurityStatus, Security},
...
mod tests {
    ...
    fn test_parse_initial_status() {
        ...
        // Errors
        let _ = parse_initial_status(&vec!["FOO:20:".to_string()]).unwrap_err();
        let _ = parse_initial_status(&vec!["FOO:20".to_string()]).unwrap_err();
        let _ = parse_initial_status(&vec![":20:1234".to_string()]).unwrap_err();
        let _ = parse_initial_status(&vec!["".to_string()]).unwrap_err();
        let _ = parse_initial_status(&vec!["FOO:asd:100".to_string()]).unwrap_err();
        let _ = parse_initial_status(&vec!["FOO:20:sdf".to_string()]).unwrap_err();
        let _ = parse_initial_status(&vec!["FOO:20:-19".to_string()]).unwrap_err();
        let _ = parse_initial_status(&vec!["FOO:-20:10".to_string()]).unwrap_err();
    }
}
