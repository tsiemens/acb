...
const GET_SUMMARY_RANGE_DELTA_INDICIES_WARN: &str =
    "No transactions in the summary period";

fn get_summary_range_delta_indicies(
    latest_date: Date,
    deltas: &Vec<TxDelta>,
    ...
    let mut first_superficial_loss_period_day =
        Date::from_calendar_date(3000, time::Month::January, 1).unwrap();
    ...
    for delta in &deltas[latest_delta_in_summary_range_idx + 1..] {
        if delta.is_superficial_loss() {
            first_superficial_loss_period_day =
                get_first_day_in_superficial_loss_period(delta.tx.settlement_date);
            tx_in_summary_overlaps_superficial_loss =
                latest_in_summary_date >= first_superficial_loss_period_day;
    ...
    if tx_in_summary_overlaps_superficial_loss {
        ...
        for i in (0..=latest_delta_in_summary_range_idx).rev() {
            ...
            if delta.is_superficial_loss() {
                ...
                first_superficial_loss_period_day =
                    get_first_day_in_superficial_loss_period(
                        delta.tx.settlement_date,
                    );
            }
        }
        debug!(
            "   latestSummarizableDeltaIdx: {:?} ({:?})",
...
fn make_simple_summary_txs(
    af: &Affiliate,
    deltas: &Vec<TxDelta>,
    latest_summarizable_delta_idx: usize,
) -> (Vec<Tx>, Vec<Warning>) {
    ...
    if let Ok(share_balance) = PosDecimal::try_from(*sum_post_status.share_balance) {
        let summary_tx = Tx {
            ...
            action_specifics: super::TxActionSpecifics::Buy(super::BuyTxSpecifics {
...
fn make_annual_gains_summary_txs(
    ...
    if !af.registered() {
        for delta in deltas[..latest_summarizable_delta_idx + 1].iter() {
            ...
            if !delta.capital_gain.unwrap_or(Decimal::ZERO).is_zero() {
                let prev_cap_gain =
                    yearly_cap_gains.get(&year).map(|g| *g).unwrap_or(Decimal::ZERO);
                yearly_cap_gains
                    .insert(year, prev_cap_gain + delta.capital_gain.unwrap());
            }
            latest_year_delta.insert(year, delta);
        }
    }

    let mut sorted_years_with_gains: Vec<i32> =
        yearly_cap_gains.keys().cloned().collect();
    ...
    let base_acb_per_share = match sum_post_status.total_acb {
        Some(total_acb) => {
            match PosDecimal::try_from(*sum_post_status.share_balance) {
    ...
    let n_base_shares = sum_post_status.share_balance
        + GreaterEqualZeroDecimal::try_from(Decimal::from(
            sorted_years_with_gains.len(),
        ))
        .unwrap();
    if let Ok(n_base_shares) = PosDecimal::try_from(*n_base_shares) {
        ...
        let dt = Date::from_calendar_date(first_year - 1, time::Month::January, 1)
            .unwrap();
        ...
        let (gain, loss) = if is_negative(&gain_or_loss) {
            (
                ...
                GreaterEqualZeroDecimal::try_from(
                    gain_or_loss * Decimal::NEGATIVE_ONE,
                )
                .unwrap(),
            )
        } else {
...
fn make_summary_txs(
    ...
    let summary_range = match get_summary_range_delta_indicies(latest_date, deltas) {
        ...
        None => {
            return (
                Vec::new(),
                vec![GET_SUMMARY_RANGE_DELTA_INDICIES_WARN.to_string()],
            );
        }
    };
    ...
    if let Some(latest_summarizable_delta_idx) =
        summary_range.latest_summarizable_delta_idx
    {
    ...
    for af in affils {
        let affil_last_summarizable_delta_idx =
            affil_last_summarizable_delta_idxs[&af];

        let (af_sum_txs, warns) = if split_annual_gains {
            make_annual_gains_summary_txs(
                &af,
                deltas,
                affil_last_summarizable_delta_idx,
            )
        } else {
    ...
    tracing::trace!(
        "make_summary_txs:   unsummarizable_deltas.len = {}",
        unsummarizable_deltas.len()
    );
            ...
            Some(sfl) => {
                ...
                match &mut unsum_tx.action_specifics {
                    crate::portfolio::TxActionSpecifics::Sell(sell_specs) => {
                        sell_specs.specified_superficial_loss = Some(SFLInput {
                            superficial_loss: LessEqualZeroDecimal::from(
                    ...
                    _ => {
                        panic!(
                            "Superficial loss was not sell, but was {}",
                            unsum_tx.action()
                        );
                    }
                }
    ...
    // last summary day.
    {
        ...
        let last_affecting_day = get_last_day_in_superficial_loss_period(
            get_last_day_in_superficial_loss_period(
                last_summarizable_delta.tx.settlement_date,
            ),
        );
...
mod tests {
    ...
    impl TSumGainsTx {
        pub fn x(&self) -> Tx {
            ...
            let (amount, commission) = if self.gain >= Decimal::ZERO {
                (
                    GreaterEqualZeroDecimal::try_from(*self.acb_per_sh + self.gain)
                        .unwrap(),
                    GreaterEqualZeroDecimal::zero(),
                )
            } else {
                (
                    self.acb_per_sh,
                    GreaterEqualZeroDecimal::try_from(
                        self.gain * Decimal::NEGATIVE_ONE,
                    )
                    .unwrap(),
                )
            };
            ...
            for i in 0..exp_txs.len() {
                ...
                if exp_tx.memo.starts_with(MATCHING_MEMO_PREFIX) {
                    let re = regex::Regex::new(
                        exp_tx.memo.split_at(MATCHING_MEMO_PREFIX.len()).1,
                    )
                    .unwrap();
                    fixed_up_tx.memo = match re.is_match(actual_tx.memo.as_str()) {
                        true => actual_tx.memo.clone(),
                        false => format!(
                            "TEST_MEMO_MATCH_FAIL: actual: '{}' did not match {:?}",
                            actual_tx.memo, re
                        ),
                    }
                }
    ...
    fn test_summary_basic() {
        ...
        let txs = vec![
            TTx{s_yr: 2021, s_doy: 4, act: A::Buy, shares: gez!(10), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
        ];
        let exp_summary_txs = vec![
            TSimpleSumTx{year: 2021, doy: 4, shares: gez!(10), amount: gez!(1.2),
                         ..def()}.x(), // commission is added to share ACB
        ];

        let deltas = txs_to_ok_delta_list(&txs);
        let (summary_txs, warnings) =
            make_summary_txs(doy_date(2022, -1), &deltas, false);
        ...
        let txs = vec![
            TTx{s_yr: 2022, s_doy: 4, act: A::Buy, shares: gez!(10), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
        ];
        ...
        let txs = vec![
            TTx{s_yr: 2022, s_doy: 4, act: A::Buy, shares: gez!(10), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2022, s_doy: 41, act: A::Sell, shares: gez!(5),
                price: gez!(0.2),
                ..def()}.x(), // SFL
        ];
        ...
        let txs = vec![
            TTx{s_yr: 2022, s_doy: 40, act: A::Buy, shares: gez!(10),
                price: gez!(1), comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2022, s_doy: 41, act: A::Sell, shares: gez!(5),
                price: gez!(0.2), ..def()}.x(), // SFL
        ];
        ...
        let txs = vec![
            TTx{s_yr: 2022, s_doy: -2, act: A::Buy, shares: gez!(10),
                price: gez!(1), comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2022, s_doy: -1, act: A::Sell, shares: gez!(2),
                price: gez!(0.2), ..def()}.x(), // SFL
        ];
        let exp_summary_txs = vec![
            TSimpleSumTx{year: 2022, doy: -1, shares: gez!(8), amount: gez!(1.45),
                         ..def()}.x(),
        ];
        ...
        let txs = vec![
            ...
            TTx{s_yr: 2022, s_doy: -1, act: A::Sell, shares: gez!(2),
                price: gez!(0.2), ..def()}.x(), // SFL
            TTx{s_yr: 2022, s_doy: 2, act: A::Sell, shares: gez!(2), price: gez!(2),
                ..def()}.x(),    // Gain
        ];
        let exp_summary_txs = vec![
            TSimpleSumTx{year: 2022, doy: -1, shares: gez!(8), amount: gez!(1.45),
        ...
        let txs = vec![
            TTx{s_yr: 2022, s_doy: -2, act: A::Buy, shares: gez!(10), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2022, s_doy: -1, act: A::Sell, shares: gez!(2), price: gez!(2),
                ..def()}.x(), // Gain
            TTx{s_yr: 2022, s_doy: 30, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                ..def()}.x(), // SFL
            TTx{s_yr: 2022, s_doy: 31, act: A::Buy, shares: gez!(1), price: gez!(2),
                comm: gez!(2), ..def()}.x(), // Causes SFL
        ];
        let exp_summary_txs = vec![
            TSimpleSumTx{year: 2022, doy: -1, shares: gez!(8), amount: gez!(1.2),
                         ..def()}.x(),
        ];
        ...
        let txs = vec![
            ...
            TTx{s_yr: 2022, s_doy: -1, act: A::Sell, shares: gez!(2), price: gez!(2),
                ..def()}.x(), // Gain
            TTx{s_yr: 2022, s_doy: 29, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                ..def()}.x(), // SFL
            TTx{s_yr: 2022, s_doy: 31, act: A::Buy, shares: gez!(1), price: gez!(2),
                comm: gez!(2), ..def()}.x(), // Causes SFL
        ];
        let exp_summary_txs = vec![
            TSimpleSumTx{year: 2022, doy: -2, shares: gez!(10), amount: gez!(1.2),
                         ..def()}.x(),
            TTx{s_yr: 2022, s_doy: -1, act: A::Sell, shares: gez!(2), price: gez!(2),
                ..def()}.x(), // Gain
        ];
        ...
        // The post SFL will influence the summarizable TXs
        let txs = vec![
            TTx{s_yr: 2022, s_doy: -1, act: A::Buy, shares: gez!(10), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2022, s_doy: -1, act: A::Sell, shares: gez!(2), price: gez!(2),
                ..def()}.x(),               // Gain
            TTx{s_yr: 2022, s_doy: 29, act: A::Sell, shares: gez!(2), price: gez!(0.2),
        ...
        let txs = vec![
            TTx{s_yr: 2022, s_doy: -32, act: A::Buy, shares: gez!(8), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2022, s_doy: -31, act: A::Buy, shares: gez!(2), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2022, s_doy: -1, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                ..def()}.x(), // SFL
            TTx{s_yr: 2022, s_doy: 29, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                ..def()}.x(), // SFL
            TTx{s_yr: 2022, s_doy: 31, act: A::Buy, shares: gez!(1), price: gez!(2),
        ...
        let exp_summary_txs = vec![
            TSimpleSumTx{year: 2022, doy: -32, shares: gez!(8), amount: gez!(1.25),
                         ..def()}.x(),
            TTx{s_yr: 2022, s_doy: -31, act: A::Buy, shares: gez!(2), price: gez!(1),
                comm: gez!(2), ..def()}.x(), // ACB of 14 total after here.
            TTx{s_yr: 2022, s_doy: -1, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                sfl: cadsfl(lez!(-2.4), false), ..def()}.x(), // SFL of $2.4
        ...
        // Unsummarizable SFL will push back the summarizing window.
        let txs = vec![
            TTx{s_yr: 2022, s_doy: -71, act: A::Buy, shares: gez!(10), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2022, s_doy: -70, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                ..def()}.x(), // SFL
            // unsummarizable below
            TTx{s_yr: 2022, s_doy: -45, act: A::Buy, shares: gez!(8), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2022, s_doy: -31, act: A::Buy, shares: gez!(2), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2022, s_doy: -15, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                ..def()}.x(), // SFL
            TTx{s_yr: 2022, s_doy: -1, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                ..def()}.x(),  // SFL
        ...
        let exp_summary_txs = vec![
            TSimpleSumTx{year: 2022, doy: -70, shares: gez!(8), amount: gez!(1.45),
                ..def()}.x(),
            TTx{s_yr: 2022, s_doy: -45, act: A::Buy, shares: gez!(8), price: gez!(1),
                comm: gez!(2), ..def()}.x(), // post ACB = 21.6
            TTx{s_yr: 2022, s_doy: -31, act: A::Buy, shares: gez!(2), price: gez!(1),
                comm: gez!(2), ..def()}.x(), // post ACB = 25.6
            TTx{s_yr: 2022, s_doy: -15, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                sfl: cadsfl(lez!(-2.4444444444444444444444444444), false), ..def()
            }.x(), // ACB = 22.755555556
            make_sfla_tx_yd(2022, -15, gez!(2.4444444444444444444444444444)),                                                                              // ACB of 25.2
            TTx{s_yr: 2022, s_doy: -1, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                sfl: cadsfl(lez!(-2.75), false), ..def()}.x(), // ACB = 22.05
        ...
        let txs = vec![
            TTx{s_yr: 2022, s_doy: -6, act: A::Buy, shares: gez!(10), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2022, s_doy: -5, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                ..def()}.x(), // SFL
            // end of summary period
            TTx{s_yr: 2022, s_doy: 26, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                ..def()}.x(), // SFL
            TTx{s_yr: 2022, s_doy: 31, act: A::Buy, shares: gez!(1), price: gez!(2),
                comm: gez!(2), ..def()}.x(), // Causes SFL
        ];
        let exp_summary_txs = vec![
            TSimpleSumTx{year: 2022, doy: -5, shares: gez!(8), amount: gez!(1.45),
                         ..def()}.x(),
        ];
        ...
        let txs = vec![
            TTx{s_yr: 2022, s_doy: -34, act: A::Buy, shares: gez!(1), price: gez!(1),
                comm: gez!(1), ..def()}.x(),
            TTx{s_yr: 2022, s_doy: -33, act: A::Buy, shares: gez!(9), price: gez!(1),
                comm: gez!(1), ..def()}.x(),
            // unsummarizable below
            TTx{s_yr: 2022, s_doy: -20, act: A::Buy, shares: gez!(4), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2022, s_doy: -2, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                ..def()}.x(), // SFL
            TTx{s_yr: 2022, s_doy: -1, act: A::Buy, shares: gez!(2), price: gez!(0.2),
                comm: gez!(2), ..def()}.x(),
            // end of summary period
            TTx{s_yr: 2022, s_doy: 2, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                ..def()}.x(),             // SFL
            TTx{s_yr: 2022, s_doy: 3, act: A::Buy, shares: gez!(1), price: gez!(2),
                comm: gez!(2), ..def()}.x(), // Causes SFL
        ];
        let exp_summary_txs = vec![
            TSimpleSumTx{year: 2022, doy: -33, shares: gez!(10), amount: gez!(1.2),
                ..def()}.x(),
            TTx{s_yr: 2022, s_doy: -20, act: A::Buy, shares: gez!(4), price: gez!(1),
                comm: gez!(2), ..def()}.x(), // ACB = 18
            TTx{s_yr: 2022, s_doy: -2, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                sfl: cadsfl(lez!(-2.1714285714285714285714285714), false), ..def()
            }.x(), // ACB = 15.428571429
        ...
        let txs = vec![
            TTx{s_yr: 2022, s_doy: -33, act: A::Buy, shares: gez!(10), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2022, s_doy: -20, act: A::Buy, shares: gez!(10), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2022, s_doy: -2, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                ..def()}.x(), // SFL
            TTx{s_yr: 2022, s_doy: -1, act: A::Buy, shares: gez!(2), price: gez!(0.6),
                comm: gez!(2), ..def()}.x(),
            // end of summary period
            TTx{s_yr: 2022, s_doy: 30, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                ..def()}.x(),             // SFL
        ...
        let txs = vec![
            TTx{s_yr: 2021, s_doy: 4, act: A::Buy, shares: gez!(10), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2021, s_doy: 4, act: A::Sell, shares: gez!(10), price: gez!(1),
                ..def()}.x(),
        ];
        ...
        let txs = vec![
            TTx{s_yr: 2022, s_doy: -33, act: A::Buy, shares: gez!(10), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2022, s_doy: -33, act: A::Sell, shares: gez!(10), price: gez!(2),
                ..def()}.x(), // Gain
            // unsummarizable below
            TTx{s_yr: 2022, s_doy: -20, act: A::Buy, shares: gez!(4), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
    ...
    fn test_summary_year_splits() {
        ...
        // Unsummarizable SFL will push back the summarizing window.
        let txs = vec![
            TTx{s_yr: 2018, s_doy: 30, act: A::Buy, shares: gez!(8), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2020, s_doy: 30, act: A::Buy, shares: gez!(8), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2020, s_doy: 31, act: A::Sell, shares: gez!(1), price: gez!(2),
                ..def()}.x(),               // GAIN
            TTx{s_yr: 2020, s_doy: 100, act: A::Sell, shares: gez!(1), price: gez!(0.9),
                ..def()}.x(),            // LOSS
            TTx{s_yr: 2021, s_doy: 100, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                ..def()}.x(),            // LOSS
            TTx{s_yr: 2022, s_doy: -1, act: A::Sell, shares: gez!(2), price: gez!(0.2),
                ..def()}.x(),             // SFL
            TTx{s_yr: 2022, s_doy: 29, act: A::Sell, shares: gez!(2), price: gez!(0.2),
        ...
        let exp_summary_txs = vec![
            TSumBaseBuyTx{year: 2017, shares: gez!(14), amount: summary_acb,
                ..def()}.x(), // shares = final shares (12) + N years with gains (2)
    ...
    fn test_multi_affiliate_summary() {
        ...
        let txs = vec![
            TTx{s_yr: 2018, s_doy: 30, act: A::Buy, shares: gez!(8), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2018, s_doy: 31, act: A::Buy, shares: gez!(4), price: gez!(1),
                comm: gez!(2), af_name: "B", ..def()}.x(),
            TTx{s_yr: 2019, s_doy: 31, act: A::Buy, shares: gez!(3), price: gez!(1),
                comm: gez!(2), af_name: "(R)", ..def()}.x(),
            TTx{s_yr: 2020, s_doy: 29, act: A::Buy, shares: gez!(5), price: gez!(1),
                comm: gez!(2), af_name: "B", ..def()}.x(),
        ];
        ...
        let exp_summary_txs = vec![
            TSumBaseBuyTx{year: 2017, shares: gez!(9),
                amount: gez!(1.4444444444444444444444444444)/*13.0 / 9.0*/,
                af_name: "B"}.x(),
            TSumBaseBuyTx{year: 2017, shares: gez!(8), amount: gez_10_o_8,
                          af_name: ""}.x(),
            // Registered accounts use 0 rather than NaN in the summary
            TSumBaseBuyTx{year: 2017, shares: gez!(3), amount: gez!(0),
                          af_name: "(R)"}.x(),
        ];
        ...
        let (summary_txs, warnings) =
            make_summary_txs(doy_date(2022, -1), &deltas, true);
        assert_vecr_eq(&warnings, &Vec::new());
        validate_txs(&exp_summary_txs, &summary_txs);
        ...
        let txs = vec![
            ...
            TTx{s_yr: 2018, s_doy: 30, act: A::Buy, shares: gez!(8), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2018, s_doy: 31, act: A::Buy, shares: gez!(7), price: gez!(1.1),
                comm: gez!(2), af_name: "B", ..def()}.x(),
            TTx{s_yr: 2019, s_doy: 31, act: A::Buy, shares: gez!(6), price: gez!(1.2),
                comm: gez!(2), af_name: "(R)", ..def()}.x(),
            // Sells
            TTx{s_yr: 2019, s_doy: 5, act: A::Sell, shares: gez!(1), price: gez!(2),
                ..def()}.x(),
            TTx{s_yr: 2019, s_doy: 6, act: A::Sell, shares: gez!(1), price: gez!(2.1),
                af_name: "B", ..def()}.x(),
            TTx{s_yr: 2020, s_doy: 7, act: A::Sell, shares: gez!(1), price: gez!(2.2),
                af_name: "(R)", ..def()}.x(),
            TTx{s_yr: 2021, s_doy: 7, act: A::Sell, shares: gez!(1), price: gez!(2.3),
                ..def()}.x(),
            TTx{s_yr: 2022, s_doy: 7, act: A::Sell, shares: gez!(1), price: gez!(2.4),
                af_name: "B", ..def()}.x(),
            TTx{s_yr: 2022, s_doy: 8, act: A::Sell, shares: gez!(1), price: gez!(2.5),
                af_name: "(R)", ..def()}.x(),
        ];
        ...
        let exp_summary_txs = vec![
            TSumBaseBuyTx{year: 2017, shares: gez!(7), amount: b_share_acb,
                          af_name: "B", ..def()}.x(),
            TSumBaseBuyTx{year: 2017, shares: gez!(8), amount: def_share_acb,
                          ..def()}.x(),
            TSumBaseBuyTx{year: 2017, shares: gez!(4), amount: gez!(0),
                          af_name: "(R)", ..def()
                        }.x(), // No gains on (R), so only the base

            TSumGainsTx{year: 2019, acb_per_sh: b_share_acb,
                        gain: dec!(2.1) - *b_share_acb, af_name: "B", ..def()}.x(),
            TSumGainsTx{year: 2019, acb_per_sh: def_share_acb,
                        gain: dec!(2.0) - *def_share_acb, ..def()}.x(),
            TSumGainsTx{year: 2021, acb_per_sh: def_share_acb,
                        gain: dec!(2.3) - *def_share_acb, ..def()}.x(),
            TSumGainsTx{year: 2022, acb_per_sh: b_share_acb,
                        gain: dec!(2.4) - *b_share_acb, af_name: "B", ..def()}.x(),
        ];
        let deltas = txs_to_ok_delta_list(&txs);
        ...
        let exp_summary_txs = vec![
            ...
            TSumBaseBuyTx{year: 2017, shares: gez!(6), amount: gez!(0),
                          af_name: "(R)", ..def()
                        }.x(), // No gains on (R), so only the base
            TSumGainsTx{year: 2019, acb_per_sh: def_share_acb,
                        gain: dec!(2.0) - *def_share_acb, ..def()}.x(),
        ...
        let deltas = txs_to_ok_delta_list(&txs);
        let (summary_txs, warnings) =
            make_summary_txs(doy_date(2024, -1), &deltas, true);
        assert_vecr_eq(&warnings, &Vec::new());
        validate_txs(&exp_summary_txs, &summary_txs);
        ...
        let txs = vec![
            ...
            TTx{s_yr: 2019, s_doy: 31, act: A::Buy, shares: gez!(6), price: gez!(1.2),
                comm: gez!(2), af_name: "(R)", ..def()}.x(),
            TTx{s_yr: 2019, s_doy: 40, act: A::Buy, shares: gez!(5), price: gez!(1.3),
                comm: gez!(2), af_name: "B (R)", ..def()}.x(),
            // Sells
            TTx{s_yr: 2020, s_doy: 5, act: A::Sell, shares: gez!(2), price: gez!(2),
                af_name: "B", ..def()}.x(),
            TTx{s_yr: 2020, s_doy: 6, act: A::Sell, shares: gez!(3), price: gez!(2),
                af_name: "B (R)", ..def()}.x(),
        ];

        let exp_summary_txs = vec![
            TSimpleSumTx{year: 2018, doy: 30, shares: gez!(8), amount: gez_10_o_8,
                         ..def()}.x(),
            TSimpleSumTx{year: 2019, doy: 31, shares: gez!(6), amount: gez!(0),
                         af_name: "(R)", ..def()}.x(),
            TSimpleSumTx{year: 2020, doy: 5, shares: gez!(5), amount: gez_9_7_o_7,
                         af_name: "B", ..def()}.x(),
            TSimpleSumTx{year: 2020, doy: 6, shares: gez!(2), amount: gez!(0),
                         af_name: "B (R)", ..def()}.x(),
        ];
        let deltas = txs_to_ok_delta_list(&txs);
        let (summary_txs, warnings) =
            make_summary_txs(doy_date(2024, -1), &deltas, false /* year gains */);
        assert_vecr_eq(&warnings, &Vec::new());
        validate_txs(&exp_summary_txs, &summary_txs);
        ...
        let txs = vec![
            ...
            TTx{s_yr: 2019, s_doy: 31, act: A::Buy, shares: gez!(6), price: gez!(1.2),
                comm: gez!(2), af_name: "(R)", ..def()}.x(),
            // Sells
            TTx{s_yr: 2019, s_doy: 5, act: A::Sell, shares: gez!(7), price: gez!(2),
                af_name: "B", ..def()}.x(),
            TTx{s_yr: 2019, s_doy: 5, act: A::Sell, shares: gez!(6), price: gez!(2),
                af_name: "(R)", ..def()}.x(),
        ];

        let b_share_acb = gez_9_7_o_7; // 9.7 / 7.0
        let exp_summary_txs = vec![
            TSumBaseBuyTx{year: 2017, shares: gez!(1), amount: gez!(0),
                          af_name: "B", ..def()}.x(),
            TSumBaseBuyTx{year: 2017, shares: gez!(8),
                          amount: gez!(1.25) /* 10.0 / 8 */ , ..def()}.x(),

            TSumGainsTx{year: 2019, acb_per_sh: gez!(0),
                        gain: crate::util::math::maybe_round_to_effective_cent(
                            (dec!(2.0) - *b_share_acb) * dec!(7.0)),
                        af_name: "B", ..def()}.x(),
        ];
        let deltas = txs_to_ok_delta_list(&txs);
        let (summary_txs, warnings) = make_summary_txs(doy_date(2024, -1), &deltas, true);
        ...
        let txs = vec![
            ...
            TTx{s_yr: 2020, s_doy: 5, act: A::Sell, shares: gez!(2), price: gez!(2),
                af_name: "B", ..def()}.x(),
            // ^^ Summarizable period ^^
            TTx{s_yr: 2020, s_doy: 101, act: A::Buy, shares: gez!(2), price: gez!(1),
                af_name: "B", ..def()}.x(),
            TTx{s_yr: 2020, s_doy: 103, act: A::Buy, shares: gez!(3), price: gez!(1),
                af_name: "C", ..def()}.x(),
            // ^^ Requested summary period ^^
            TTx{s_yr: 2020, s_doy: 105, act: A::Sell, shares: gez!(2), price: gez!(0.9),
                sfl: cadsfl(lez!(-0.751020), false), af_name: "B", ..def()}.x(),
        ];
        let exp_summary_txs = vec![
        ...
        let txs = vec![
            ...
            TTx{s_yr: 2018, s_doy: 31, act: A::Buy, shares: gez!(7), price: gez!(1.1),
                comm: gez!(2), af_name: "(R)", ..def()}.x(),
            TTx{s_yr: 2020, s_doy: 59, act: A::Sell, shares: gez!(2), price: gez!(2),
                ..def()}.x(),
            TTx{s_yr: 2020, s_doy: 60, act: A::Sell, shares: gez!(2), price: gez!(1.1),
                comm: gez!(2), af_name: "(R)", ..def()}.x(),
            // ^^ Summarizable period ^^
            TTx{s_yr: 2020, s_doy: 102, act: A::Sell, shares: gez!(3), price: gez!(1.1),
                comm: gez!(2), af_name: "(R)", ..def()}.x(),
            TTx{s_yr: 2020, s_doy: 103, act: A::Buy, shares: gez!(3), price: gez!(2),
                ..def()}.x(),
            // ^^ Requested summary period ^^
            TTx{s_yr: 2020, s_doy: 105, act: A::Sell, shares: gez!(2), price: gez!(0.9),
                sfl: cadsfl(lez!(-1.2), false), ..def()}.x(),
        ];
        let exp_summary_txs = vec![
            TSimpleSumTx{year: 2020, doy: 59, shares: gez!(6), amount: gez_10_o_8, ..def()}.x(),
            TSimpleSumTx{year: 2020, doy: 60, shares: gez!(5), amount: gez!(0),
                         af_name: "(R)", ..def()}.x(),
            // ^^ Summarizable period ^^
            TTx{s_yr: 2020, s_doy: 102, act: A::Sell, shares: gez!(3), price: gez!(1.1),
                comm: gez!(2), af_name: "(R)", ..def()}.x(),
        ...
        let txs = vec![
            TTx{s_yr: 2020, s_doy: 50, act: A::Buy, shares: gez!(7), price: gez!(1.1),
                comm: gez!(2), af_name: "(R)", ..def()}.x(),
            TTx{s_yr: 2020, s_doy: 51, act: A::Buy, shares: gez!(8), price: gez!(1),
                comm: gez!(2), ..def()}.x(),
            TTx{s_yr: 2020, s_doy: 59, act: A::Sell, shares: gez!(2), price: gez!(2),
                ..def()}.x(),
            TTx{s_yr: 2020, s_doy: 60, act: A::Sell, shares: gez!(1), price: gez!(1.1),
                comm: gez!(2), af_name: "(R)", ..def()}.x(),
            TTx{s_yr: 2020, s_doy: 70, act: A::Sell, shares: gez!(1), price: gez!(1.1),
                comm: gez!(2), af_name: "(R)", ..def()}.x(),
            // ^^ Summarizable period ^^
            TTx{s_yr: 2020, s_doy: 85, act: A::Sell, shares: gez!(3), price: gez!(1.1),
                comm: gez!(2), af_name: "(R)", ..def()}.x(),
            TTx{s_yr: 2020, s_doy: 103, act: A::Buy, shares: gez!(3), price: gez!(2),
                ..def()}.x(),
            // ^^ Requested summary period ^^
        ...
        let exp_summary_txs = vec![
            TSimpleSumTx{year: 2020, doy: 59, shares: gez!(6), amount: gez_10_o_8,
                         ..def()}.x(),
            TSimpleSumTx{year: 2020, doy: 70, shares: gez!(5), amount: gez!(0),
                         af_name: "(R)", ..def()}.x(),
            // ^^ Summarizable period ^^
            TTx{s_yr: 2020, s_doy: 85, act: A::Sell, shares: gez!(3), price: gez!(1.1),
                comm: gez!(2), af_name: "(R)", ..def()}.x(),
        ...
        let txs = vec![
            ...
            TTx{s_yr: 2020, s_doy: 103, act: A::Buy, shares: gez!(3), price: gez!(2),
                ..def()}.x(),
            // ^^ Requested summary period ^^
            TTx{s_yr: 2020, s_doy: 105, act: A::Sell, shares: gez!(2), price: gez!(1.1),
                comm: gez!(2), af_name: "(R)", ..def()}.x(),
        ];
        let exp_summary_txs = vec![
            TSimpleSumTx{year: 2020, doy: 102, shares: gez!(2), amount: gez!(0),
                         af_name: "(R)", ..def()}.x(),
            TSimpleSumTx{year: 2020, doy: 103, shares: gez!(9), amount: gez!(1.5),
                         ..def()}.x(),
            // ^^ Summarizable period ^^
            // ^^ Requested summary period ^^
        ];
