...
impl AffiliatePortfolioSecurityStatuses {
    ...
    pub fn new(
        security: String,
        initial_default_aff_status: Option<Rc<PortfolioSecurityStatus>>,
        ...
        if let Some(init_status) = initial_default_aff_status {
            ...
            assert_eq!(
                init_status.share_balance,
                init_status.all_affiliate_share_balance
            );
    ...
    pub fn get_latest_post_status_for_affiliate(
        &self,
        af: &Affiliate,
    ...
    fn make_default_portfolio_security_status(
        &self,
        af: &Affiliate,
    ...
    pub fn get_latest_post_status(&self) -> Rc<PortfolioSecurityStatus> {
        match self.get_latest_post_status_for_affiliate(&self.latest_affiliate) {
            Some(s) => s.clone(),
            None => Rc::new(
                self.make_default_portfolio_security_status(&self.latest_affiliate),
            ),
        }
    }
    ...
    pub fn set_latest_post_status(
        ...
        let expected_all_share_bal = *v.share_balance
    ...
    pub fn get_next_pre_status(
        &self,
        af: &Affiliate,
    ) -> Rc<PortfolioSecurityStatus> {
        ...
        if last_status_for_af.all_affiliate_share_balance
            == self.latest_all_affiliates_share_balance
        {
            ...
            let mut next_pre_status: PortfolioSecurityStatus =
                (*last_status_for_af).clone();
            next_pre_status.all_affiliate_share_balance =
                self.latest_all_affiliates_share_balance;
...
pub mod testlib {
    ...
    use crate::{
        portfolio::PortfolioSecurityStatus, util::decimal::GreaterEqualZeroDecimal,
    };
        ...
        pub fn x(&self) -> Rc<PortfolioSecurityStatus> {
            Rc::new(PortfolioSecurityStatus {
                ...
                total_acb: if let Some(aps) = self.acb_per_sh {
                    Some(
                        GreaterEqualZeroDecimal::try_from(*aps * *self.shares)
                            .unwrap(),
                    )
                } else {
                    self.total_acb
...
mod tests {
    ...
    fn test_full_use_case() {
        ...
        assert_eq!(
            statuses.get_latest_post_status_for_affiliate(&default_af),
            None
        );
        ...
        let latest_def =
            statuses.get_latest_post_status_for_affiliate(&default_af).unwrap();
        assert_eq!(
            TPSS {
                shares: gez!(2),
                all_shares: gez!(2),
