...
pub async fn load_tx_rates(
    ...
    for tx in csv_txs {
        ...
        .await
        .map_err(|e| format!("Commission exchange rate error: {}", e))?;
...
mod tests {
    ...
    use crate::{
        fx::{io::testlib::new_test_rate_loader, DailyRate},
        portfolio::{io::tx_loader::load_tx_rates, CsvTx, Currency},
        testlib::assert_re,
    ...
    fn test_load_tx_rates() {
        ...
        let (mut rate_loader, _, remote_rates) = new_test_rate_loader(false);
        remote_rates
            .borrow_mut()
            .insert(2024, vec![dr(doy_date(2024, 10), dec!(1.2))]);
        ...
        let mut csv_txs = vec![
            ...
            CsvTx {
                trade_date: odate_yd(2024, 11),
                tx_currency: cad.clone(),
                commission_currency: cad.clone(),
                ..CsvTx::default()
            },
            ...
            CsvTx {
                trade_date: odate_yd(2024, 11),
                tx_currency: usd.clone(),
                tx_curr_to_local_exchange_rate: Some(dec!(1.6)),
                commission_currency: usd.clone(),
                ..CsvTx::default()
            },
            CsvTx {
                ...
                tx_currency: usd.clone(),
                commission_currency: usd.clone(),
                commission_curr_to_local_exchange_rate: Some(dec!(1.5)),
                ..CsvTx::default()
            },
            // Explicit (with un-fetchable date)
            CsvTx {
                trade_date: odate_yd(2024, 300),
                tx_currency: usd.clone(),
                tx_curr_to_local_exchange_rate: Some(dec!(1.7)),
                commission_currency: usd.clone(),
                commission_curr_to_local_exchange_rate: Some(dec!(1.8)),
                ..CsvTx::default()
            },
        ];

        async_std::task::block_on(load_tx_rates(&mut csv_txs, &mut rate_loader))
            .unwrap();
        // USD
        assert_eq!(csv_txs[0].tx_curr_to_local_exchange_rate, Some(dec!(1.2)));
        assert_eq!(
            csv_txs[0].commission_curr_to_local_exchange_rate,
            Some(dec!(1.2))
        );
        // CAD
        ...
        assert_eq!(
            csv_txs[4].commission_curr_to_local_exchange_rate,
            Some(dec!(1.2))
        );
        ...
        assert_eq!(
            csv_txs[5].commission_curr_to_local_exchange_rate,
            Some(dec!(1.2))
        );

        assert_eq!(csv_txs[6].tx_curr_to_local_exchange_rate, Some(dec!(1.2)));
        assert_eq!(
            csv_txs[6].commission_curr_to_local_exchange_rate,
            Some(dec!(1.5))
        );
        ...
        assert_eq!(
            csv_txs[7].commission_curr_to_local_exchange_rate,
            Some(dec!(1.8))
        );
    }
    ...
    fn test_load_tx_rates_errors() {
        ...
        let err = do_error_test(CsvTx {
            trade_date: odate_yd(2024, 150),
            tx_currency: usd.clone(),
            ..CsvTx::default()
        });
        assert_re(
            r"^Exchange rate error: Unable to retrieve exchange rate for 2024-05-30",
            &err,
        );
        ...
        assert_re(
            r"^Commission exchange rate error: Unable to retrieve exchange rate for 2024-05-30",
            &err,
        );
    }
}
