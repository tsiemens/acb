...
use acb::{
    ...
    util::{date::parse_standard_date, http::standalone::StandaloneAppRequester, rw::{DescribedReader, WriteHandle}}
};
...
fn validate_sample_csv_file(
    ...
    async_std::task::block_on(
        run_acb_app_to_render_model(
            vec![reader],
            HashMap::new(),
            &TxCsvParseOptions::default(),
            false,
            render_costs,
            RateLoader::new(false,
                Box::new(CsvRatesCache::new(cache_dir.to_path_buf(), err_stream.clone())),
...
fn do_test_sample_csv_file_validity(render_costs: bool) {
    ...
    validate_sample_csv_file(
        Path::new("./test/test_combined.csv"), &dir.path, render_costs);
    validate_sample_csv_file(
        Path::new("./www/html/sample_txs.csv"), &dir.path, render_costs);
}
